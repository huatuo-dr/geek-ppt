...
在 `src/services/renderService.ts` 的 `render()` 函数中添加新插件的样式和包裹类：

```typescript
import { getNeonStyles, getNeonWrapperClass } from "@/plugins/neon/neonStyles";
...
} else if (req.pluginId === "neon-renderer") {
  css = getNeonStyles();
  wrapperClass = getNeonWrapperClass();
} else {
  css = getCoolStyles();
